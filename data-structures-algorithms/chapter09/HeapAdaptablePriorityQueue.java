...
public class HeapAdaptablePriorityQueue<K,V> extends HeapPriorityQueue<K,V> implements AdaptablePriorityQueue<K,V> {
  ...
  protected static class AdaptablePQEntry<K,V> extends PQEntry<K,V> {
    ...
    /** Get index of entry in heap */
    public int getIndex() {
      return index;
    }

    /** Set index of entry in heap */
  ...
  /** Creates an empty adaptable priority queue using given comparator */
  ...
  protected void bubble(int j) {
    if (j > 0 && compare(heap.get(j), heap.get(parent(j))) < 0) {
      ...
      downheap(j); // althought it might not need to move
    }
  }
